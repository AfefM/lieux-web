  ...
  return (
    <main className="bg-black min-h-screen">
      <header className="fixed top-0 w-full z-50 p-8 flex justify-between items-center mix-blend-difference">
        <div className="font-bold tracking-[0.3em] text-[10px] uppercase text-white">Skander Zarrad</div>
      ...
      <section className="relative h-[85vh] w-full flex items-center justify-center overflow-hidden">
        <Image src={lieux[0].image} alt="Hero" fill className="object-cover opacity-40 grayscale" priority />
        <h1 className="relative z-10 text-[22vw] font-black leading-none tracking-tighter opacity-80 select-none">LIEUX</h1>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {lieux.map((lieu, index) => (
            <div key={lieu.id} className={`group relative overflow-hidden bg-zinc-900 ${index === 0 ? 'md:col-span-2 aspect-[21/9]' : 'aspect-[4/5]'}`}>
              <Image src={lieu.image} alt={lieu.title[currentLang]} fill className="object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-1000 opacity-70 group-hover:opacity-100" />
