    ...
    <main className="bg-black text-white min-h-screen selection:bg-white selection:text-black font-sans">
      ...
      <header className="fixed top-0 w-full z-50 px-6 md:px-12 py-8 flex justify-between items-center bg-black/60 backdrop-blur-md border-b border-white/10">
        ...
        <nav className="flex gap-8 text-[10px] uppercase tracking-[0.3em] font-black text-white">
          ...
          <Link href="#" className="bg-white text-black px-5 py-2 hover:bg-zinc-200 transition-all rounded-sm">Contact</Link>
        </nav>
      </header>

      {/* HERO SECTION */}
      <section className="relative h-[70vh] flex items-center justify-center overflow-hidden border-b border-white/20 bg-zinc-900">
        ...
        <div className="relative z-10 text-center">
          <motion.h1 
            ...
            className="text-[16vw] font-black leading-none tracking-tighter uppercase text-white"
          >
            Lieux<span className="text-white/40">.</span>
          </motion.h1>
          <p className="text-[14px] tracking-[1.5em] uppercase text-white mt-6 font-black ml-[1.5em] drop-shadow-lg">
            Archives Visuelles
          </p>
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex gap-8 overflow-x-auto no-scrollbar w-full md:w-auto">
            {categories.map((cat) => (
              <button
                ...
                className={`text-[11px] uppercase tracking-[0.2em] font-black transition-all relative pb-2 ${
                  activeCategory === cat ? "text-white" : "text-white/40 hover:text-white"
                }`}
              >
                {cat}
                {activeCategory === cat && (
                  <motion.div layoutId="nav-line" className="absolute bottom-0 left-0 w-full h-[2px] bg-white" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="relative flex items-center w-full md:w-80 border-b border-white/40 focus-within:border-white transition-colors">
            ...
            <input 
              ...
              className="bg-transparent text-[11px] uppercase tracking-[0.3em] pl-8 py-2 focus:outline-none w-full text-white placeholder:text-white/30 font-bold"
              ...
              >
                <Link href={`/fr/lieux/${lieu.slug}`}>
                  <div className="relative aspect-[3/2] overflow-hidden bg-zinc-900 border border-white/20 shadow-2xl">
                    ...
                    />
                    {lieu.isPrintAvailable && (
                      <div className="absolute bottom-3 right-3 bg-white text-black p-2 rounded-sm shadow-xl">
                  ...
                  <div className="mt-6 space-y-3">
                    <div className="flex justify-between items-start">
                      <h3 className="text-[14px] font-black uppercase tracking-[0.1em] leading-tight text-white transition-colors">
                        {lieu.title.fr}
                      </h3>
      ...
      {/* FOOTER AVEC LIEN VERS GIRAAF PARK */}
      <footer className="py-32 px-6 md:px-12 border-t border-white/20 bg-black flex flex-col items-center">
        <div className="max-w-[1400px] w-full flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
          ...
          <div className="text-[12px] tracking-[0.5em] text-white font-black uppercase">
            © 2025 SKANDER ZARRAD
          </div>

          <div className="flex flex-col items-center md:items-end gap-2">
            <div className="text-[10px] tracking-[0.3em] text-white/60 uppercase font-bold">
              Designed & Developed by
            </div>
            
            {/* LIEN DE VOTRE ENTREPRISE ICI */}
            <Link 
              href="https://giraafpark.com" 
              target="_blank" 
              className="group flex flex-col items-center md:items-end"
            >
              <span className="text-[18px] font-black tracking-[0.2em] uppercase text-white group-hover:text-zinc-400 transition-colors">
                GIRAAF PARK
              </span>
