...
export default function Page() {
  // On force la langue à 'fr' pour cette page spécifique
  const currentLang = 'fr';

  // Sécurité si les données sont vides ou mal chargées
  if (!lieux || lieux.length === 0) {
    return (
      <div className="bg-black min-h-screen text-white flex items-center justify-center font-light tracking-widest">
        CHARGEMENT...
      </div>
    );
  }
  ...
  return (
    <main className="bg-black min-h-screen text-white font-sans selection:bg-white selection:text-black">
      ...
      {/* HEADER FIXE - Effet Mix-Blend pour le style minimaliste */}
      <header className="fixed top-0 w-full z-50 p-8 md:p-12 flex justify-between items-center mix-blend-difference text-white">
        <div className="font-bold tracking-[0.4em] text-[10px] md:text-[12px] uppercase">
          Skander Zarrad
        </div>
        <nav className="flex gap-8 text-[10px] md:text-[11px] tracking-[0.2em] uppercase font-black">
           <Link href="#" className="opacity-20 hover:opacity-100 transition-opacity">AR</Link>
           <Link href="/fr" className="border-b border-white pb-1">FR</Link>
           <Link href="#" className="opacity-20 hover:opacity-100 transition-opacity">EN</Link>
        </nav>
      </header>

      {/* SECTION HERO - Impact visuel fort */}
      <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        <h1 className="relative z-10 text-[24vw] font-black leading-none tracking-tighter opacity-90 select-none mix-blend-overlay">
            LIEUX
        </h1>
      ...
      {/* SECTION GRILLE - Portfolio des projets */}
      <section className="max-w-[1600px] mx-auto py-32 px-6 md:px-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20">
          {lieux.map((lieu, index) => (
            <div 
              key={lieu.id} 
              className={`group relative overflow-hidden bg-zinc-950 transition-all duration-700 ${
                index === 0 ? 'md:col-span-2 aspect-[21/9]' : 'aspect-[4/5]'
              }`}
            >
              {/* Image avec transition Grayscale -> Couleur au survol */}
              <Image 
                src={lieu.image} 
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-8 md:p-16 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
